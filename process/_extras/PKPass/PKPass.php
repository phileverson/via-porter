...
class PKPass {
	 ...
	 */
	public function create($output = false, $ourPassID, $i_passSet) {
		$paths = $this->paths();
...
// mkdir($ourPassID . '-' . rand(0, 100000000000), 0777, true);
$passMade = file_get_contents($paths['pkpass']);
$passFileName = $ourPassID . '.pkpass';

file_put_contents($passFileName, $passMade);
rename($passFileName, '_passes/' . $ourPassID . '/' . $i_passSet . '/' . $passFileName);
